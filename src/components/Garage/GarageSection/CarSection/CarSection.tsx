...
const CarSection: React.FC<CarSectionProps> = ({ onSelectCar, car }) => {
	...
	const isOverheated = useSelector(
		(state: RootState) =>
			state.cars.cars.find((c) => c.id === car.id)?.isOverheated
	);
		...
		>
			...
			>
				<svg
					...
					viewBox='0 0 110 80'
				>
					<g fill={car.color} transform='scale(-1, 1) translate(-100, 0)'>
				...
				<p style={{ color: '#ffffff' }}>
					{`${car.name} ${isOverheated ? '  is overheated!' : ''}`}
				</p>
			</div>
		</div>
	);
